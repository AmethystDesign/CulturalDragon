...
// Variable: ITEMS_QUERY
// Query: *[_type == "item" && defined(slug.current) && !defined($search) || title match $search || category match $search || author->name match $search] | order(_createdAt desc) {    _id,     title,     slug,    _createdAt,    author -> {      _id,       name,       image,       bio    },     views,    description,    category,    image,  }
export type ITEMS_QUERYResult = Array<{
  _id: string;
  title: string | null;
  slug: Slug | null;
  _createdAt: string;
...
// Variable: ITEMS_BY_AUTHOR_QUERY
// Query: *[_type == "item" && author._ref == $id] | order(_createdAt desc) {  _id,   title,   slug,  _createdAt,  author -> {    _id, name, image, bio  },   views,  description,  category,  image,}
export type ITEMS_BY_AUTHOR_QUERYResult = Array<{
  _id: string;
  title: string | null;
  slug: Slug | null;
  _createdAt: string;
...
// Variable: ITEM_BY_ID_QUERY
// Query: *[_type == "item" && _id == $id][0]{  _id,   title,   slug,  _createdAt,  author -> {    _id, name, username, image, bio  },   views,  description,  category,  image,  details,}
export type ITEM_BY_ID_QUERYResult = {
  _id: string;
  title: string | null;
  slug: Slug | null;
  _createdAt: string;
...
// Variable: ITEM_VIEWS_QUERY
// Query: *[_type == "item" && _id == $id][0]{        _id, views    }
export type ITEM_VIEWS_QUERYResult = {
  _id: string;
  views: number | null;
} | null;
...
// Variable: PLAYLIST_BY_AUTHOR_QUERY
// Query: *[_type == "playlist" && slug.current == $slug][0]{  _id,  title,  slug,  select[]->{    _id,    _createdAt,    title,    slug,    author->{      _id,      name,      slug,      image,      bio    },    views,    description,    category,    image,    details  }}
export type PLAYLIST_BY_AUTHOR_QUERYResult = {
  _id: string;
  title: string | null;
  slug: Slug | null;
  select: Array<{
